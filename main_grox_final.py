...
# Optional GitHub/Render integration for generated software bots.
# These are intentionally optional so Grox can still run for website-only work.
RENDER_API_KEY = os.getenv("RENDER_API_KEY")
GITHUB_TOKEN = os.getenv("GITHUB_TOKEN")
GITHUB_REPO = os.getenv("GITHUB_REPO")
GITHUB_BRANCH = os.getenv("GITHUB_BRANCH", "main")

# Optional Telegram credentials passed to generated Telegram projects.
...
class Order:
    ...
    # Generated software-bot deployment information.
...
async def gemini_request(
    ...
    # GEMINI_RETRIES is the number of retries after the first request.
    total_attempts = max(1, GEMINI_RETRIES + 1)

    for attempt in range(1, total_attempts + 1):
        try:
            print(
                f"[GEMINI] Attempt {attempt}/{total_attempts} "
                f"model={GEMINI_MODEL}"
            )
            ...
            if response is None:
                raise RuntimeError("Gemini повернув None.")
            ...
            print(
                f"[GEMINI ERROR] Attempt {attempt}/{total_attempts}: "
            ...
            delay = min(
                60,
                (2 ** (attempt - 1)) + random.uniform(0.5, 2.0)
            )

            print(
                f"[GEMINI] Тимчасова помилка. "
    ...
    raise RuntimeError(
        "Gemini тимчасово недоступний після "
        f"{total_attempts} спроб. "
        f"Остання помилка: "
...
async def generate_test_and_fix_website(
    ...
    for attempt in range(
        ...
        try:
            ...
            await thread.send(
                "⚠️ Gemini не зміг виконати "
                "виправлення на цій спробі.\n\n"
                f"🔄 Повторюю. Наступна спроба: "
                f"{attempt + 1}/{MAX_WEBSITE_FIX_ATTEMPTS}"
            )
...
# ============================================================
# SOFTWARE BOT GENERATION + GITHUB + RENDER
# ============================================================

def is_software_bot_project(task: str) -> bool:
    ...
    return any(word in normalized for word in bot_words)


def detect_bot_platform(task: str) -> str:
    normalized = task.casefold()
...
async def generate_software_project(client_task: str):
    platform = detect_bot_platform(client_task)

    prompt = f"""
Ти — senior software developer системи Grox.

Створи ПОВНІСТЮ ГОТОВИЙ до запуску програмний проєкт
за технічним завданням клієнта.
...
КРИТИЧНО ВАЖЛИВО:
- Реалізуй саме функції з ТЗ.
- Не вигадуй функції, яких клієнт не просив.
- Код має бути робочим, а не макетом.
- Не залишай TODO, pass як заміну функціональності або фальшиві кнопки.
- Проєкт повинен запускатися командою `python main.py`.
- Створи health HTTP endpoint `/`, який слухає PORT з environment variable.
  Це потрібно для запуску як Render Web Service.
- Для Discord використовуй discord.py.
- Для Telegram використовуй Telegram Bot API/aiogram, якщо ТЗ стосується
  звичайного Telegram-бота. Якщо ТЗ прямо потребує пошуку публічних
  Telegram-спільнот через MTProto, використовуй Telethon і передбач
  TELEGRAM_API_ID та TELEGRAM_API_HASH як environment variables.
- Не вставляй секретні токени в код.
- Усі токени/API keys бери з environment variables.
- Не вигадуй значення токенів.
- Створи requirements.txt з усіма потрібними залежностями.
- Створи README.md з короткими інструкціями запуску та списком
  потрібних environment variables.
- main.py має бути самодостатнім і запускати health server паралельно
  з основним ботом.
- Якщо певна функція вимагає зовнішніх credentials, код має коректно
  повідомити про їх відсутність, а не падати незрозумілою помилкою.

ПОВЕРНИ РІВНО ТРИ ФАЙЛИ В ТАКОМУ ФОРМАТІ:
...
Не використовуй Markdown fences навколо файлів.
"""
    ...
    patterns = {
        "main.py": r"===FILE:\s*main\.py===\s*(.*?)(?=\n===FILE:|\Z)",
        "requirements.txt": r"===FILE:\s*requirements\.txt===\s*(.*?)(?=\n===FILE:|\Z)",
        "README.md": r"===FILE:\s*README\.md===\s*(.*?)(?=\n===FILE:|\Z)",
    }
    ...
    if "main.py" not in files or "requirements.txt" not in files:
        raise RuntimeError(
            "Gemini не повернув повний software-проєкт "
...
async def github_upsert_file(
    ...
    if not GITHUB_TOKEN:
        raise RuntimeError("GITHUB_TOKEN не налаштований.")

    if not GITHUB_REPO:
        raise RuntimeError("GITHUB_REPO не налаштований.")
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        async with session.get(
            ...
            if response.status == 200:
                ...
                raise RuntimeError(
                    f"GitHub GET failed HTTP {response.status}: {body}"
                )
        ...
        async with session.put(
            ...
            if response.status not in (200, 201):
                raise RuntimeError(
                    f"GitHub PUT failed HTTP {response.status}: {body}"
                )
...
async def deploy_project_to_render(
    service_name: str,
    root_dir: str
):
    if not RENDER_API_KEY:
        raise RuntimeError(
            "RENDER_API_KEY не налаштований."
        )

    if not GITHUB_REPO:
        raise RuntimeError(
            "GITHUB_REPO не налаштований."
        )
    ...
    async with aiohttp.ClientSession(
        ...
        # Render requires ownerId when creating a service.
        async with session.get(
            f"{api_url}/owners",
            headers=headers
        ) as response:
            owners_body = await response.text()

            if response.status != 200:
                raise RuntimeError(
                    f"Render owners request failed "
                    f"HTTP {response.status}: {owners_body}"
                )

            owners_data = json.loads(owners_body)

        owners = owners_data.get("owners", owners_data)

        if not owners:
            raise RuntimeError(
                "Render API не повернув жодного owner."
            )

        owner_id = (
            owners[0].get("owner", {}).get("id")
            or owners[0].get("id")
        )

        if not owner_id:
            raise RuntimeError(
                "Не вдалося визначити ownerId Render."
            )
        ...
        async with session.post(
            ...
            if response.status not in (200, 201):
                raise RuntimeError(
                    f"Render service creation failed "
                    f"HTTP {response.status}: {body}"
                )

            data = json.loads(body)

    # Render can wrap a service in a `service` object.
...
async def build_and_deploy_software_bot(
    order: Order,
    thread: discord.Thread
):
    if not GITHUB_TOKEN or not GITHUB_REPO:
        await thread.send(
            "⚠️ **Не можу автоматично опублікувати бота.**\n\n"
            "Для автоматичного створення потрібні "
            "GITHUB_TOKEN та GITHUB_REPO у Render Environment."
        )
        return False

    if not RENDER_API_KEY:
        await thread.send(
            "⚠️ **Проєкт можу згенерувати, але не можу "
            "автоматично запустити його в Render.**\n\n"
            "У Render Environment відсутній RENDER_API_KEY."
        )
        return False

    await thread.send(
        "🤖 **Генерую програмного бота...**\n\n"
        "🧠 Створюю код, залежності та інструкції."
    )

    files = await generate_software_project(
        order.task
    )
    ...
    await thread.send(
        "📦 **Публікую проєкт у GitHub...**"
    )

    for filename, content in files.items():
        await github_upsert_file(
            root_dir,
            f"{root_dir}/{filename}",
            content,
            f"Grox: generate order #{order.order_id} - {filename}"
        )

    order.project_repo_path = root_dir

    await thread.send(
        "☁️ **Створюю окремий Render-сервіс для бота...**"
    )

    render_info = await deploy_project_to_render(
        f"grox-order-{order.order_id}",
        root_dir
    )

    order.render_service_id = render_info.get("id")
    order.deployment_url = render_info.get("url")

    message = (
        "✅ **Програмний бот створений і відправлений у Render.**\n\n"
        f"📁 GitHub: `{GITHUB_REPO}/{root_dir}`\n"
    )

    if order.deployment_url:
        message += (
            f"🌐 Render: {order.deployment_url}\n"
        )

    message += (
        "\n⚠️ Для Telegram/Discord ботів реальна робота "
        "може додатково залежати від потрібних токенів "
        "та API-доступів, які не можна вбудовувати в код."
    )
...
async def process_order(
    ...
    try:
        ...
        elif is_software_bot_project(order.task):
            try:
                success = await build_and_deploy_software_bot(
                    order,
                    thread
                )
...
async def on_ready():
    ...
    print(
        f"🤖 Software bot generation: "
        f"{'YES' if GITHUB_TOKEN and GITHUB_REPO and RENDER_API_KEY else 'NO'}"
    )
